...
Shader "Custom/Mosaic2"{
	Properties {
		_CellSize ("Cell Size", Range(0, 10)) = 3
	...
	SubShader {
		...
		void surf (Input i, inout SurfaceOutputStandard o) {
			...
			float3 rg = rand1dTo1d(noise.y);

			
			float valueChange = fwidth(value.z) * 0.5;
			float weight = 1 - smoothstep(0.05 - valueChange, 0.05 + valueChange, noise.z);
			float3 cellColor = lerp(_MainColor, _TintColor, noise.x);
			float3 color = lerp(cellColor, _TintColor, noise.y);
			
			o.Albedo =  color * (0.5 + (1-noise.x));// * cellColor;
			o.Alpha = .5;
		}
		ENDCG
	}
